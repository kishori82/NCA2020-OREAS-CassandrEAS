...
\author{\IEEEauthorblockN{Kishori M. Konwar*, Muriel Medard}
\IEEEauthorblockA{\textit{Dept of EECS, MIT} \\
Cambridge, USA \\
\{kishori, medard\}@mit.edu}
\and
\IEEEauthorblockN{Haochen Pan, Lewis Tseng, Yingjian Wu}
\IEEEauthorblockA{\textit{Department of Computer Science, Boston College} \\
%\textit{Boston College}\\
Boston,USA \\
\{haochen.pan, lewis.tseng, wuit\}@bc.edu}
}
\maketitle
...
%
		In this work, we propose a erasure coding-based protocol that implements a key-value store with atomicity and \textit{near-optimal storage cost}. 
	...
	\end{abstract}
\begin{IEEEkeywords}
 Strong consistency, fault-tolerence, distributed storage
\end{IEEEkeywords}
	\input{intro}

	\input{preliminaries}

    \input{models-and-definitions-v1}

	\input{two-round-algorithm}
     
	\input{one-round-algorithm}
	%\input{design-kmk-v1}
	%\input{evaluation}
