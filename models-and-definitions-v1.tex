...
carefully designed algorithms. Moreover, there are some lower bound results on the  storage-cost for erasure-coded atoomic memory algorithms in the presence of faulty clients~\cite{cadambe2016information, SCCK15}. As a result, to  circumvent this restriction many authors assume some restricting assumptions.  CASGC, ORCAS-A and ORCAS-B~\cite{DGL08} assume a bound on the number of concurrent operation. In the same vein, \treasmod{} assumes a known bound on the number of concurrent writes with a read to achieve liveness. 
